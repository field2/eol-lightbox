jQuery(document).ready(function($) {
	// global variables for script
	var current, size;
	$('.blocks-gallery-item a').click(function(e) {
		// prevent default click event
		e.preventDefault();
		// scroll to the top of the page if they've scrolled down
var scroll = $(window).scrollTop();
console.log("windowscroll is " + scroll);
if(scroll !== 0) {
	$(window).scrollTop(0);
}
		// get href from clicked element
		var image_href = $(this).attr("href");  
		console.log(image_href);
		// determine the index of clicked trigger
		var slideNum = $('.blocks-gallery-item a').index(this);
				console.log(slideNum);

		// find out if #ggb_lightbox exists
		if ($('#ggb_lightbox').length > 0) {        
			// if it does
			$('#ggb_lightbox').fadeIn(300);
			// otherwise
		} else {                                
			// create HTML markup for lightbox window
			var lightbox =
					'<div id="ggb_lightbox">' +
					'<a href="#" class="ggb_close"></a>' +
					'<ul id="ggb_slider"></ul>' +        
										'<div class="ggb_nav">' +
					'<a href="#" class="ggb_prev ggb_slide-nav"></a>' +
					'<a href="#" class="ggb_next ggb_slide-nav"></a>' +
					'</div><!-- .ggb_nav -->' +
					'</div><!-- #ggb_lightbox -->';
			// add lightbox HTML to the DOM
			$('body').append(lightbox);
			// make sure the user can't scroll while the lightbox is showing
			$('body').addClass("ggb_noscroll");
			// fill lightbox with .blocks-gallery-grid a hrefs
			$('.wp-block-gallery').find('.blocks-gallery-item a').each(function() {
				var $href = $(this).attr('href');
				$('#ggb_slider').append(
					'<li>' +
					'<img src="' + $href + '">' +
					'</li>'
				);
			});
		}
		// set the slider size based on number of objects in slideshow
		size = $('#ggb_slider > li').length;
		// hide all slides, then show the selected slide
		$('#ggb_slider > li').hide();
		$('#ggb_slider > li:eq(' + slideNum + ')').show();
		// set current to selected slide
		current = slideNum;
	});
	// click anywhere on the page to get rid of lightbox window
	$('body').on('click', '#ggb_lightbox', function() { // using .on() instead of .live(). more modern, and fixes event bubbling issues
		$('#ggb_lightbox').fadeOut(300);
					$('body').removeClass("ggb_noscroll");
	});
	// navigation prev/next
	$('body').on('click', '.ggb_slide-nav', function(e) {
		// prevent link from firing and lightbox from accidentally closing
		e.preventDefault();
		e.stopPropagation();
		var $this = $(this);
		var dest;
		// check if prev arrow was clicked
		if ($this.hasClass('ggb_prev')) {
			dest = current - 1;
			if (dest < 0) {
				dest = size - 1;
			}
		} else {
			// otherwise they clicked next
			dest = current + 1;
			if (dest > size - 1) {
				dest = 0;
			}
		}
		// fadeOut current slide, fadeIn next/prev slide
		$('#ggb_slider > li:eq(' + current + ')').fadeOut(750);
		$('#ggb_slider > li:eq(' + dest + ')').fadeIn(750);
		// update current slide
		current = dest;
	});
});